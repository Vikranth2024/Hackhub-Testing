...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await loginUser (loginData); // Call the loginUser  function
      console.log('Login successful:', response);
      navigate('/dashboard'); // Redirect to dashboard on successful login
    } catch (error) {
      console.error('Login failed:', error);
      setFormErrors((prev) => ({ ...prev, general: error.message || 'Login failed' }));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleMetaMaskLogin = async () => {
    if (typeof window.ethereum !== 'undefined') {
      ...
      try {
        ...
        const signature = await web3.eth.personal.sign(message, address, '');
        console.log('MetaMask Login:', { address, signature, message, nonce });
        // Here you can send the address and signature to your backend for verification
        navigate('/dashboard');
      } catch (error) {
        console.error('MetaMask login failed:', error);
      }
  ...
  return (
    <div className="space-y-6">
      ...
      {loginMethod === 'metamask' && (
        <div className="space-y-6">
          <p className="text-sm text-center">Connect your MetaMask wallet to authenticate with your DID-NFT.</p>
          <Button onClick={handleMetaMaskLogin} fullWidth variant="outline" isLoading={isLoading}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {userInputs.map((word, index) => (
              <input
                key={index}
                type="text"
                className="border rounded-md w-full px-3 py-2 text-center"
                value={word}
